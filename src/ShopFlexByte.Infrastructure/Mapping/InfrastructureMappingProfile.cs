...
internal sealed class InfrastructureMappingProfile : Profile
{
    public InfrastructureMappingProfile()
    {
        ...
        ShouldMapProperty = p =>
            !p.Name.StartsWith("Nav", StringComparison.Ordinal);
    ...
    private void ConfigureOrderMappings()
    {
        ...
        CreateMap<Persistence.Entities.OrderItem, OrderItem>()
            .ConstructUsing(i =>
                new OrderItem(i.ProductId, i.ProductName, i.ProductPrice, i.Quantity));

        CreateMap<OrderItem, Persistence.Entities.OrderItem>();
    }

    private static Order CreateOrder(Persistence.Entities.Order src, ResolutionContext resolutionContext) =>
        new(
            src.UserId,
            src.Items.Select(i =>
                new OrderItem(i.ProductId, i.ProductName, i.ProductPrice, i.Quantity))
    ...
    private static void HydrateCartItems(
        Persistence.Entities.ShoppingCart src,
        ShoppingCart dest)
    {
