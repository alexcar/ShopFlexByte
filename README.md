...
| G2. Modelar aplicações utilizando Domain-Driven Design <br> O aluno diferenciou claramente Domain Services e Factories na modelagem do domínio, justificando sua escolha com base na responsabilidade de cada elemento? | ![Address.cs](https://github.com/alexcar/ShopFlexByte/blob/main/src/ShopFlexByte.Domain/ValueObjects/Address.cs) <br> ![FullName.cs](https://github.com/alexcar/ShopFlexByte/blob/main/src/ShopFlexByte.Domain/ValueObjects/FullName.cs) |
| H2. Modelar aplicações utilizando Domain-Driven Design <br> O aluno modelou o domínio considerando a integração entre Bounded Contexts, aplicando padrões como Anti-Corruption Layer e Context Map com clareza e eficácia? | ![InfrastructureMappingProfile.cs](https://github.com/alexcar/ShopFlexByte/blob/main/src/ShopFlexByte.Infrastructure/Mapping/InfrastructureMappingProfile.cs)  |
| I3. Criar aplicações empregando padrões de projeto - SOLID e GRASP <br> O aluno aplicou os princípios SOLID no design das classes, garantindo coesão, alta responsabilidade e baixo acoplamento? | ![CreateCustomerUseCase](https://github.com/alexcar/ShopFlexByte/blob/main/src/ShopFlexByte.Application/UseCases/ManageCustomer/CreateCustomerUseCase.cs) |
| J3. Criar aplicações empregando padrões de projeto - SOLID e GRASP <br> O aluno utilizou corretamente o princípio de Single Responsibility nas classes, evitando a concentração excessiva de responsabilidades? | ![DeleteCustomerUseCase.cs](https://github.com/alexcar/ShopFlexByte/blob/main/src/ShopFlexByte.Application/UseCases/ManageCustomer/DeleteCustomerUseCase.cs) |
| K3. Criar aplicações empregando padrões de projeto - SOLID e GRASP <br> O aluno aplicou o padrão Low Coupling para garantir a independência entre as classes e promover a reutilização de código? | ![RepositoryBase.cs](https://github.com/alexcar/ShopFlexByte/blob/main/src/ShopFlexByte.Infrastructure/Persistence/Repositories/RepositoryBase.cs) |
| L3. Criar aplicações empregando padrões de projeto - SOLID e GRASP <br> O aluno utilizou o padrão Controller de forma adequada, promovendo a separação entre lógica de controle e demais responsabilidades? | ![ProcessPaymentUseCase.cs](https://github.com/alexcar/ShopFlexByte/blob/main/src/ShopFlexByte.Application/UseCases/ProcessPayment/ProcessPaymentUseCase.cs) |
| M4. Desenvolver testes unitários e aplicar TDD <br> O aluno aplicou corretamente os princípios de testes unitários como isolamento, repetibilidade, rapidez, auto-verificação e abrangência? | ![AddItemToCartUseCaseTests.cs](https://github.com/alexcar/ShopFlexByte/blob/main/tst/ShopFlexByte.Application.UnitTests/UseCases/AddItemToCartUseCaseTests.cs) |
| N4. Desenvolver testes unitários e aplicar TDD <br> O aluno implementou testes unitários abrangendo todos os métodos que contêm regras de negócio relevantes? | ![AccessCustomerDataUseCaseTests.cs](https://github.com/alexcar/ShopFlexByte/blob/main/tst/ShopFlexByte.Application.UnitTests/UseCases/AccessCustomerDataUseCaseTests.cs) |
| O4. Desenvolver testes unitários e aplicar TDD <br> O aluno utilizou mocks e stubs de maneira adequada para isolar o código sob teste durante a implementação dos testes unitários? | ![ProcessPaymentUseCaseTests.cs](https://github.com/alexcar/ShopFlexByte/blob/main/tst/ShopFlexByte.Application.UnitTests/UseCases/ProcessPaymentUseCaseTests.cs) |
| P4. Desenvolver testes unitários e aplicar TDD <br> O aluno implementou testes unitários com cobertura superior a 80% do código de domínio, garantindo qualidade e confiabilidade da aplicação? | ![ShoppingCartTests.cs](https://github.com/alexcar/ShopFlexByte/blob/main/tst/ShopFlexByte.Domain.UnitTests/Entities/ShoppingCartTests.cs) |
